const { skillRatioWeight: srw, skillMaxXP, skillOverflowMultipliers, skillFactors } = require("./constants.json");
...
const getSkillWeight = (skillLevels, skillXP) => {
    ...
    for (let i = 0; i < skillXP.length; i++) {
        if (skillXP[i] > skillMaxXP) {
            ...
            if (t > 0)
                overflowRating += (rating * overflowMult);
        }
    }
