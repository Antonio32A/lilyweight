...
class LilyWeight {
    ...
    async getWeight (player, returnUsername = false) {
        return getWeight(player.length > 16 ? player : await getUUID(player), this.apiKey, returnUsername);
    }
    ...
    async getWeightFromUUID (uuid, returnUsername = false) {
        return getWeight(uuid, this.apiKey, returnUsername);
    }
    ...
    async getWeightFromUsername  (username, returnUsername = false) {
        return getWeight(await getUUID(username), this.apiKey, returnUsername);
    }

    static getWeightRaw = getWeightRaw; 
}
